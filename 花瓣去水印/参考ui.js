...
(function () {
    ...
    function getConfig() {
        return {
            ...
            historyLoadingStyle: GM_getValue('historyLoadingStyle', DEFAULT_CONFIG.historyLoadingStyle)
        };
    }
    ...
    function getDownloadHistory() { try { const list = GM_getValue('downloadHistory', []); return Array.isArray(list) ? list : []; } catch (e) { return []; } }
    function saveDownloadHistory(list) { try { GM_setValue('downloadHistory', list); } catch (e) { console.error(e); } }
            ...
            s.onload = () => {
                try {
                    ...
                    if (!window.pinyinPro || typeof window.pinyinPro.pinyin !== 'function') {
                        if (gw.pinyinPro && typeof gw.pinyinPro.pinyin === 'function') { window.pinyinPro = { pinyin: gw.pinyinPro.pinyin }; }
                        else if (typeof gw.pinyin === 'function') { window.pinyinPro = { pinyin: gw.pinyin }; }
                    }
                } catch (e) { }
                ready();
            };
    ...
    function addDownloadHistoryItem(item) {
        ...
        const record = {
            ...
            fileName: item.fileName, url: item.url, pageUrl: item.pageUrl || location.href, originHref: item.originHref || '',
            time: Date.now(), official: !!item.official, width: item.width || 0, height: item.height || 0,
            action: item.action || 'download', name_py: pyInfo.py, name_py_acronym: pyInfo.ac
        };
        list.unshift(record);
        ...
        try {
            ...
            for (let i = 0; i < list.length; i++) {
                ...
                if (!it.name_py || !it.name_py_acronym) {
                    ...
                    if (info.py || info.ac) { it.name_py = info.py; it.name_py_acronym = info.ac; changed = true; }
                }
            }
            if (changed) saveDownloadHistory(list);
        } catch (e) { }
    }
    function removeDownloadHistoryItem(id) { saveDownloadHistory(getDownloadHistory().filter(x => x.id !== id)); }
    function clearDownloadHistory() { saveDownloadHistory([]); }
    function isOfficialMaterial() {
        return (Array.from(document.querySelectorAll('.fgsjNg46')).some(el => el.textContent && el.textContent.includes('官方自营')) || document.querySelectorAll('[title="来自官方自营"]').length > 0);
    }
            ...
            try {
                ...
                (async () => {
                    ...
                    if (watermarkRegex.test(originalSrc) && !originalSrc.includes('/small/')) {
                        ...
                        if (await checkImageUrl(newSrc)) { img.src = newSrc; modified = true; }
                    }
                    if (img.srcset && watermarkRegex.test(img.srcset) && !img.srcset.includes('/small/')) {
                        ...
                        if (await checkImageUrl(newSrcset.split(' ')[0])) { img.srcset = newSrcset; modified = true; }
                    }
        ...
        function process() {
            ...
            if (viewer) {
                ...
                if (img && img.complete && img.naturalWidth > 0) {
                    ...
                    if (img.hasAttribute('data-watermark-removed') && interval) { clearInterval(interval); interval = null; }
                }
            ...
            mutations.forEach((m) => {
                if (m.addedNodes.length) {
                    m.addedNodes.forEach((n) => {
                        if (n.nodeType === 1 && (n.querySelector('#imageViewerWrapper') || n.querySelector('img.vYzIMzy2[alt="查看图片"]'))) {
                            setTimeout(() => processWatermark(true), 100);
                            if (!interval) { interval = setInterval(process, 300); setTimeout(() => { if (interval) clearInterval(interval); }, 5000); }
                        }
                    });
                }
                if (m.type === 'attributes' && m.target.tagName === 'IMG' && m.target.matches('img.vYzIMzy2[alt="查看图片"]')) {
                    setTimeout(() => processWatermark(true), 100);
                }
            });
        });
    ...
    function observePageChanges() {
        ...
        const observer = new MutationObserver((mutations) => {
            ...
            mutations.forEach(m => {
                ...
                else if (m.type === 'attributes' && m.target.tagName === 'IMG') needProcess = true;
            });
            if (needProcess && now - lastTime > 500) {
                setTimeout(() => { processWatermark(); lastTime = Date.now(); }, 100);
            }
        });
    ...
    function interceptAjaxRequests() {
        ...
        XMLHttpRequest.prototype.send = function (body) {
            this.addEventListener('load', function () {
                if (this.responseURL && this.responseURL.includes('huaban.com') && (this.responseURL.includes('/pins/') || this.responseURL.includes('/api/'))) {
    ...
    function interceptFetchRequests() {
        ...
        window.fetch = function (input, init) {
            return originalFetch.apply(this, arguments).then(res => {
                ...
                if (url && url.includes('huaban.com') && (url.includes('/pins/') || url.includes('/api/'))) {
                    setTimeout(() => processWatermark(), 300);
                }
                return res;
            });
            ...
            if (img.tagName === 'IMG' && img.src.includes('gd-hbimg-edge.huabanimg.com')) {
                ...
                if (cleanUrl !== img.src) {
                    ...
                    try {
                        ...
                        addDownloadHistoryItem({ fileName, url: cleanUrl, pageUrl: location.href, originHref, official: isOfficialMaterial(), width: w, height: h, action: 'drag' });
                    } catch (err) { }
                }
            }
        });
            ...
            if (img.tagName === 'IMG' && img.src.includes('gd-hbimg-edge.huabanimg.com') &&
                ...
                if (cleanUrl !== img.src) {
                    ...
                    setTimeout(() => {
                        ...
                        GM_download({
                            url: cleanUrl, name: fileName,
                            onload: function () {
                                const w = (img.naturalWidth) || 0; const h = (img.naturalHeight) || 0;
                                const isDetailPage = window.location.pathname.includes('/pins/');
                                const pa = img.closest('a');
                                const originHref = isDetailPage ? window.location.href : (pa ? pa.href : '');
                                addDownloadHistoryItem({ fileName: getFileNameFromAlt(img), url: cleanUrl, pageUrl: location.href, originHref, official: isOfficialMaterial(), width: w, height: h, action: 'download' });
                            }
                        });
                    }, 100);
                }
    ...
    function createConfigUI() {
        ...
        function renderContent() {
            ...
            if (currentTab === 'features') {
                ...
                appendSwitch('enableRemoveWatermark', 'VIP 素材去水印', '支持官方自营素材，替换为无水印大图');
                appendSwitch('enableDragDownload', '拖拽下载图片', '适配资源管理器 / PureRef，自动重命名');
                appendSwitch('enableRightClickDownload', '右键下载图片', '修复下载时的文件名乱码');
                ...
                `;
                loadRow.querySelector('select').value = currentConfig.historyLoadingStyle;
                loadRow.querySelector('select').onchange = (e) => currentConfig.historyLoadingStyle = e.target.value;
                ...
                subPanel.className = `hb-sub-panel ${currentConfig.enableCustom ? 'open' : ''}`;
                subPanel.innerHTML = `
                ...
                // Bind Color Pickers
                bindColorPicker(subPanel.querySelector('#wrap-material'), 'materialColor');
                ...
                createLinkBtn('📦 历史下载', '查看本地记录的下载历史', () => { showDownloadHistory(); });
                createLinkBtn('📖 使用说明', '查看详细的功能介绍与帮助', () => { showUsageGuide(); });
                createLinkBtn('🤝 网友互助', '进入 Twikoo 评论区交流', () => { showTwikooChat(); });

                const footerInfo = document.createElement('div');
                footerInfo.style.cssText = 'margin-top:20px; font-size:12px; color:#94a3b8; line-height:1.6;';
    ...
    function init() {
        ensurePinyinLib(() => { try { hydratePinyinForHistory(); } catch (e) { } });
        ...
        setInterval(() => { processWatermark(); }, 2000);

        window.addEventListener('beforeunload', () => { observer.disconnect(); });

        console.info(`%c 花瓣去水印 %c v${getScriptVersion()} `, 'padding: 2px 6px; border-radius: 3px 0 0 3px; color: #fff; background: #E60044; font-weight: bold;', 'padding: 2px 6px; border-radius: 0 3px 3px 0; color: #fff; background: #FF9999; font-weight: bold;');
    }
